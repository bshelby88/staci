...
def main() -> None:
    ...
    # Simulated payload — ONLY when explicitly enabled, and ALWAYS labeled.
    # (Previously this ran unconditionally, selling fabricated analysis to
    # paying customers. Never present simulated data as live analysis.)
    if os.environ.get("TRADINGAGENTS_MOCK") == "1":
        payload = {
            ...
            "simulated": True,
            "disclaimer": (
                "SIMULATED OUTPUT for integration testing only. No market data "
                "was analyzed. Not financial advice."
            ),
            "summary": f"Simulated placeholder response for {ticker}; the live multi-agent engine was not invoked.",
            "reports": {
                "fundamentals": "simulated — no issuer fundamentals analyzed",
                "sentiment": "simulated — no sentiment scan performed",
                "news": "simulated — no news scan performed",
                "technical": "simulated — no market data analyzed",
                "trader_plan": "simulated",
                "risk_review": "simulated",
